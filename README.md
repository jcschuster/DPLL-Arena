...
Put your provers in the folder `solver` in a format that is executable via `./<your_solver>` or as a single Python or Elixir file. After building and running the application, the results can be found in CSV-format and as visualization in the folder `results`. If your solver is not in binary format (i.e., a .py or a .exs file), make sure that your program uses _LF_ characters, not _CRLF_ (Windows standard) or _CR_ (MacOS standard)!
...
- `s SATISFIABLE` means that the prover can satisfy the problem. A variable assignment can be given with an additional line `v <VARIABLES>` where `<VARIABLES>` contains of positive and negative numbers representing the assignment (true or false) to the respective variable and may end in "0" (e.g., `v 1 -2 3 0`). Note that the correctnes of the model will be checked by the system.
